...
<p className="blog-body-lead">Merchants choose a category for every product they upload, and some of those choices are wrong. Checking each listing by hand does not scale. This article builds an assisted review pipeline on a small batch of listings: a vision model audits the title, category, and images together and flags the suspicious rows, and a similarity search then follows one confirmed mistake to the other listings that deserve the same check. Lance stores the listings, images, and audit results and answers the similarity queries; Vane Data prepares the rows, generates embeddings, calls the model, and turns the answers into structured data. Reviewers look only at the flagged rows.</p>
...
<img
  ...
  alt="Preparing listings, generating embeddings, auditing with a model, human review, and similarity search"
  width="2400"
  height="700"
  loading="lazy"
  decoding="async"
...
The scenario starts with listing `B073P3NK7T`, titled **"Table Desk Lamp With Bulb."** The title describes a complete desk lamp that includes a bulb, and the image shows the lamp itself. The listing belongs in **Table Lamp**, but the merchant filed it under **Light Bulb**.
...
The Lance provider is published on PyPI as `vane-extension-lance` and pins an exact `vane-ai` version, so one command installs the matched pair. The `google` extra supplies the Gemini client used for the audit. Pin exact versions when a reproducible deployment matters.
...
Each listing goes to the model with its title, its category name, and all of its images. The model returns `recognized_product`, `verdict`, `suggested_category`, `confidence`, and `reason`.
...
One row shows the merchant's category, the product the model recognized, the model's suggested category, the model's reason, and the original images together. After a person confirms a correction, the listing goes back to the catalog system that applies it.

## 5. Expand a confirmed mistake with similarity search

When a reviewer confirms that a listing is misplaced, it becomes a seed. Searching the same batch with the seed's title vector and image vector returns the listings worth checking next. Each search produces its own ranking; merging the two rankings by listing ID pushes listings that rank high in both searches to the top, and a hit in only one ranking is still kept as a candidate.
...
The candidates can go through the model audit again, or straight to a reviewer. Either way, one confirmed mistake now points at the other listings that deserve the same check.

## What the run produces

The pipeline runs end to end on the seven complete listings, with Jina embeddings and Gemini audit, on the default Ray runner. The verdicts are:
...
The demonstration run uses placeholder images, so the model judged mostly from titles and the image side of the audit contributes little. With real ABO images, the image vectors and the image part of the audit participate fully. The pipeline itself — vectors into Lance, structured audit output, and the two-way rank merge — does not depend on the images and is verified end to end.

## Conclusion

Lance stores and retrieves; Vane Data understands and calls models. Listings and images stop being something a person merely glances at and become data that can be queried, reviewed, and reused. The model flags, the similarity search points at candidates, and the reviewer moves from every listing to only the suspicious ones.

The listing data comes from the Amazon Berkeley Objects (ABO) dataset, licensed under CC BY 4.0. The merchant, the upload dates, and the batch-wide misclassification story are scenario settings, not real ABO events.
