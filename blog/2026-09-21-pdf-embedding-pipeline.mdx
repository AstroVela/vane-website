...
# The file list files (an Arrow table) is defined separately.
...
chunk_schema = {
    "id": vane.sqltypes.VARCHAR,
    "source": vane.sqltypes.VARCHAR,
    "page": vane.sqltypes.INTEGER,
    "text": vane.sqltypes.VARCHAR,
}
vector_schema = {
    **chunk_schema,
    "vector": vane.array_type(vane.sqltypes.FLOAT, 384),
}
...
Here, `schema` is a dictionary of output column names to Vane types, not a `pyarrow.Schema`. `pdf_chunks(row)` receives one input row as a dictionary and yields chunk dictionaries; `Embedder.__call__(table)` receives a `pyarrow.Table` and must return all five output columns, including the metadata. The `vector` column must use Arrow `float32` elements with 384 values per row, matching `vane.array_type(vane.sqltypes.FLOAT, 384)`. Assign every chunk a non-null string `id` that is unique across the entire input, for example from the document path, page number, and chunk index.

Install the Milvus client dependency with `pip install 'vane-ai[milvus]'` and create a matching collection before running either example. `MilvusSink` performs full-row upserts and does not create the collection. This example assumes a local Milvus service and one schedulable GPU; `gpus=1.0` reserves the GPU resource, while the user-defined `Embedder` must load its model onto CUDA. With remote workers, use a Milvus endpoint reachable from every worker instead of `localhost`.
