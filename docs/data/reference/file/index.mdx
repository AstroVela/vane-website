...
File functions construct and inspect `FILE`-family values: `FILE`, `IMAGEFILE`, `AUDIOFILE`, and `VIDEOFILE`. They resolve storage and metadata without decoding media, and back the `file` argument used by the image, audio, and video functions. Most functions have a Python form, a SQL form, and an `Expression` method; `from_files` and `open_file` are Python-only.
...
`ImageFile.decode()` / `metadata()`, [AudioFile](../media/audio/index.mdx) methods, and [VideoFile](../media/video/index.mdx) methods always use their Python implementations.
...
- [Media reference](../media/index.mdx)
