...
Vane carries multimodal data as typed engine values: the `FILE` family for immutable object references, `IMAGE` for decoded pixels, and `TENSOR` for numeric arrays including audio waveforms. Six engine types cover files, images, audio, video, and tensors; all six are first-class engine types. Each domain has a Python API and a SQL API, and image, audio, and video select a `python` (default) or `native` execution backend. Expression calls return a lazy `vane.Expression`; name it with `.alias(...)`. SQL calls are projection expressions and use `AS` to name their result.
