...
`build_video_index` uses the selected backend and returns an opaque `BLOB` recording the actual presentation-order frames and their keyframe anchors. Materialize it once and store it alongside its `VIDEOFILE` before issuing repeated selections.
