...
The example uses Vane's Transformers provider. Because the AI function returns its output Relation separately, `append_embedding` validates row counts and the expected column before attaching embeddings to the original chunk rows.
...
```python
    ...
    if not args.skip_embeddings:
        ...
        embedded = append_embedding(conn, chunks, embedded_only)
...
The decoding, chunking, and embedding stages each expose an execution-backend option. Keep the sample path local while validating schemas, then select `ray_task` or `ray_actor` for expensive stages and configure the Ray runner. The source-loading and output contracts do not change.

See the [complete source](https://github.com/AstroVela/vane/blob/main/examples/common_crawl.py) for every argument and the WET parser implementation.
