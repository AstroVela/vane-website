...
StackExchange contains many lower-scoring questions even when a closely related, higher-scoring question already exists. This tutorial follows [`examples/llms_red_pajamas.py`](https://github.com/AstroVela/vane/blob/main/examples/llms_red_pajamas.py) to build a compact semantic-matching pipeline with Vane.
...
The example validates that the embedding Relation has exactly as many rows as the input and contains the requested output column before attaching it. The default model is `sentence-transformers/all-MiniLM-L6-v2`; local-files-only mode prevents model downloads when the cache is already populated.
...
Each result retains both questions, both scores, the match rank, the cosine similarity, and the matched question URL. The script converts those dictionaries back to an Arrow Relation and uses SQL to order and shorten the terminal preview.
...
The dataset load and similarity loop are intentionally straightforward for a tutorial. The embedding stage exposes local, Ray task, and Ray actor execution backends, so it is the first stage to move when model inference dominates runtime. For a much larger candidate set, replace the in-process all-pairs ranking with a vector index or a partitioned similarity strategy while preserving the row schema shown here.

See the [complete source](https://github.com/AstroVela/vane/blob/main/examples/llms_red_pajamas.py) for sample rows, output serialization, and every argument.
