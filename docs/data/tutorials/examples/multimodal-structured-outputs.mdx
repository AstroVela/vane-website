...
The default dataset is synthetic, but inference still uses an OpenAI-compatible model endpoint. By default that endpoint is the Hugging Face router and the API key is read from `HF_TOKEN`.

## 1. Define structured result contracts

The choice response contains one answer letter. The judge response separates detailed reasoning, a concise hypothesis, and a coarse attribution. Pydantic models provide the structured-output contract when the selected endpoint supports SDK parsing.
...
The synthetic source creates solid-color PNGs and a four-choice question. Because every answer truly requires the image, it is a useful smoke test for the ablation logic.
...
The AI2D path streams rows from Hugging Face, takes the first user and assistant turns, normalizes the assistant answer to A–D, converts the first image to bytes, and keeps only complete examples. Both sources become an Arrow Relation with `id`, `source`, `question`, `answer`, and binary `image` columns.

## 3. Run inference with the image

Common provider options are assembled once so the vision, text-only, and judge calls use the same model, endpoint, sampling settings, and execution backend.
...
`append_prompt_output` checks the output row count and requested column before attaching results to the source Relation by position. This catches missing or extra rows and missing output columns; it does not independently verify row order.

## 4. Repeat the question without the image

The ablation uses the already enriched Relation but does not pass `image_columns`. It writes to a separate response column and applies the same answer-normalization logic.
...
The default model is `Qwen/Qwen3-VL-8B-Instruct` through the Hugging Face OpenAI-compatible router. The base URL, API-key environment variable, model, temperature, token limits, structured-output mode, and execution backend are configurable. When using a Ray backend, workers need network access to the endpoint and the same Vane AI dependencies.
