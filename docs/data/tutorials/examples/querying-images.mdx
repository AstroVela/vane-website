...
The default backend is local. The same UDF can run as a Ray task or actor after every worker has NumPy and Pillow available.
...
See the [complete source](https://github.com/AstroVela/vane/blob/main/examples/querying_images.py) for sample-image generation, local glob loading, and the public OpenImages listing path.
