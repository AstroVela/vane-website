...
def glob_relation(conn: Any, audio_glob: str, limit: int) -> Any:
    ...
    table = pa.table(
        {
            "id": pa.array(list(range(len(paths))), type=pa.int64()),
            "path": pa.array(paths, type=pa.string()),
            "audio_format": pa.array(
                [Path(path).suffix.lstrip(".").lower() or "audio" for path in paths],
                type=pa.string(),
            ),
            ...
            "fallback_transcript": pa.array(
                [f"Audio file {Path(path).name} is ready for transcription." for path in paths],
                type=pa.string(),
            ),
        }
    )
...
```python
    transcripts = rel.map_batches(transcriber, **map_kwargs)
    transcript_table = transcripts.to_arrow_table()
...
Vane's Transformers provider then embeds the original segment text. As in the other embedding examples, the helper validates row count and the output column before appending embeddings.
...
```python
    ...
    embedded_only = embed_text(
        ...
        execution_backend=args.embedding_execution_backend,
        batch_size=args.embedding_batch_size,
    )
    embedded = append_embedding(conn, subtitle_rel, embedded_only)
...
The terminal output reports audio and subtitle row counts, then previews both summary rows and embedded segment rows. Full vectors are used in the in-memory Relation but the CSV intentionally records only their dimensions.

## Scaling and model deployment

Transcription and embedding have separate execution backends and batch sizes. For GPU speech recognition, an actor backend can reuse the cached Faster-Whisper model, while the embedding stage can use a different actor pool and resource shape. Every worker must be able to import the optional dependencies and access the model cache or model path.

See the [complete source](https://github.com/AstroVela/vane/blob/main/examples/voice_ai_analytics.py) for sample WAV generation, OpenAI summary prompting, model-loading errors, and all arguments.
