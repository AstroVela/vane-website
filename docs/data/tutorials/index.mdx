...
| Prompt and structured output | `uv pip install 'vane-ai[openai]'` and an OpenAI API key | OpenAI-compatible endpoints can be selected with provider options |
...
Run these commands from the sparse checkout root. They use built-in or generated inputs; only the Prompt example requires a hosted model by default.
...
| Prompt and structured output | `VANE_RUNNER=local python examples/basic_prompt.py` | Plain-text, typed `STRUCT`, raw-response, and SQL results in the terminal; set `OPENAI_API_KEY` first |

The other six commands leave the Vane runner unset, so the installed package default applies. Set `VANE_RUNNER=local` for the local runner, or leave it unset and set `RAY_ADDRESS` before launch to use an existing Ray cluster. The Basic Prompt command explicitly uses `local` because its checked-in single-image input comes from an in-process `read_blob` table function; stage binary input in a distributed source before adapting it to Ray. Every worker needs the same example-specific packages and access to the same data, model cache, and credentials.
...
| [Prompt and structured output](/docs/data/tutorials/examples/basic-prompt) | Generate plain text, a native typed `STRUCT`, and raw provider response JSON from prompt and image columns | Checked-in prompt and image plus a hosted model |
