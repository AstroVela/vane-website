...
From the use-case directory, run `VANE_RUNNER=local-fast .venv/bin/python src/web_text_deduplication.py`. The offline path needs no crawler or model endpoint. Its results make every reduction step concrete:
