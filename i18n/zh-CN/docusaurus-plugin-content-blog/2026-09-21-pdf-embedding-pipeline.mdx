...
这里的 `schema` 是“输出列名 → Vane 类型”的字典，不是 `pyarrow.Schema`。`pdf_chunks(row)` 接收一行输入字典，逐条 yield 片段字典；`Embedder.__call__(table)` 接收 `pyarrow.Table`，必须返回包含元数据在内的全部五列。`vector` 列的 Arrow 元素类型必须是 `float32`，每行包含 384 个值，与 `vane.array_type(vane.sqltypes.FLOAT, 384)` 一致。每个片段的字符串 `id` 必须非空且在整个输入中唯一，例如由文档路径、页码和片段序号组成。

运行前需通过 `pip install 'vane-ai[milvus]'` 安装 Milvus 客户端依赖，并提前创建字段匹配的集合。`MilvusSink` 执行整行 upsert，不会自动创建集合。示例假设 Milvus 服务位于本机且有一块可调度的 GPU；`gpus=1.0` 用于预留 GPU 资源，用户定义的 `Embedder` 仍需将模型加载到 CUDA。使用远程工作进程时，应将 `localhost` 替换为所有工作进程均可访问的 Milvus 地址。
