...
| Python Expression UDF | 使用 `vane.func`、`vane.func.batch`、`vane.cls` 或 `vane.cls.batch` 构建 Expression | 一个有类型的 projection 列；通常保持行数 | 相同的 projection 更适合用 Python 对象构建 |
| Relation UDF | 调用 `rel.map`、`rel.flat_map` 或 `rel.map_batches` | 返回 Relation；`map` 追加 `value`，`flat_map` 和 `map_batches` 定义完整输出 schema | callable 需要控制表形或行数，或者该阶段需要显式 backend、actor pool、`side_effects` 等 Relation 专属控制项 |

Python batch Expression 有一个范围很窄的例外：设置
`row_preserving=False` 时可以改变行数，但它必须是唯一的顶层 projection。
Relation UDF 目前是 Python 方法；Vane 不会把它们暴露成 SQL table function。
...
| `flat_map` | 输入一个 row dict，输出零个或多个 row dict | 可以改变行数 | 仅返回声明的 `schema` | Python `vane.func.batch(..., row_preserving=False)` 是受限的单列替代形式；v1 中 SQL 没有对应形式 |
...
基数与调用形态是两个不同的问题。Scalar Expression UDF、注册的 SQL alias
和 Relation `map` 都会为每个输入行保留一个输出行。Relation `flat_map`
明确为每个输入行生成零个或多个输出行。Relation `map_batches` 可以返回相同
或不同数量的行。

Python batch Expression 通过 `row_preserving` 声明这个契约。设置
`row_preserving=True` 后，结果可以和普通列并列 projection。设置
`row_preserving=False` 后，它可以改变行数，但必须是唯一的顶层 projection。
v1 中的 SQL 注册只接受保持行数的 batch wrapper。
...
tokenizer、client、lookup table 或 cache 时，使用有状态 class。v1 中一个
有状态 Expression 在每次 query 中使用一个 actor；Relation actor pool 可以
包含多个相互独立的 instance。
