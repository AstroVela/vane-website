...
运行这个可选阶段前，请在当前环境中安装 provider 和 schema 依赖：
...
Ray 已经是默认 runner。当 SQL 准备、UDF 阶段或模型调用需要集群 worker 时，请按照[部署](/zh-CN/docs/data/deploy/deployment)中的说明，在启动进程前设置 `RAY_ADDRESS`。
