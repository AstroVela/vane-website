...
张量函数构造和查看 `TENSOR(dtype, shape)` 值。可变形状张量存储为 `STRUCT(data LIST, shape INTEGER[rank])`，完全固定的形状使用密集 `ARRAY`。`tensor()` 始终产生可变形状张量，`tensor_data` 与 `tensor_shape` 要求可变形状张量。`tensor_type` 与存储契约见[多模态类型](types.mdx)。
