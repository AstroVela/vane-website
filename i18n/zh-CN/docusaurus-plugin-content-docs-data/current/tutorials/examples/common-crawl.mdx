...
示例使用 Vane 的 Transformers provider。由于 AI 函数会单独返回输出 Relation，`append_embedding` 会在把嵌入接回原文本块之前，检查行数以及预期列是否存在。
...
解码、切分和嵌入阶段都可以配置执行后端。先用本地样例验证 schema，再为计算密集阶段选择 `ray_task` 或 `ray_actor`，并配置 Ray runner。数据加载与输出契约保持不变。
