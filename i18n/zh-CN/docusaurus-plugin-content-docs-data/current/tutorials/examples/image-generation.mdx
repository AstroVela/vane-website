...
真实生成路径中的有状态 UDF 会推迟到首次使用时才加载模型，并在实例上缓存模型管线。对于 actor 后端，这一点尤其重要，因为后续批次可以复用模型状态。
...
命令行默认使用 512×512 输出、20 个扩散推理步骤和本地执行。占位后端会忽略仅与扩散模型有关的设置。进行真实推理时，请选择 Diffusers 后端，确保每个执行节点都能访问模型，选择合适精度，并显式请求 GPU 资源。
