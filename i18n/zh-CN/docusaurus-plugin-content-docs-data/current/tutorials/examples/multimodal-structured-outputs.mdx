...
默认通过 Hugging Face 的 OpenAI 兼容路由服务访问 `Qwen/Qwen3-VL-8B-Instruct`。基础 URL、API key 环境变量、模型、temperature、token 上限和结构化输出模式都可配置。执行方式由 Vane 配置的 runner 决定；使用 Ray 时，执行节点需要能访问模型端点，并安装相同的 Vane AI 依赖。
