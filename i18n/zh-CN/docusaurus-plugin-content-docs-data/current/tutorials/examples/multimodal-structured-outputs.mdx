...
默认数据集是合成的，但推理仍然使用 OpenAI 兼容的模型端点。默认端点是 Hugging Face 路由服务，API key 从 `HF_TOKEN` 读取。

## 1. 定义结构化结果契约

选择题响应包含一个答案字母。诊断响应把详细推理、简短假设和粗粒度归因分开。所选端点支持 SDK 解析时，Pydantic 模型会提供结构化输出契约。
...
AI2D 输入从 Hugging Face 流式读取数据行，取得第一轮用户和助手消息，把助手答案规范化为 A–D，将第一张图转换为字节，并且只保留完整样例。两个数据源都会变成包含 `id`、`source`、`question`、`answer` 和二进制 `image` 列的 Arrow Relation。

## 3. 带图像运行推理

通用 provider 选项只组装一次，使多模态、纯文本和诊断调用共享模型、端点、采样设置与执行后端。
...
`append_prompt_output` 会在按位置把结果接回源 Relation 之前，检查输出行数和请求的列。这样可以发现多行、少行或缺少输出列的情况，但不能独立验证行顺序。

## 4. 不带图像重复提问

消融阶段使用已经扩充的 Relation，但不传 `image_columns`。它写入单独的响应列，并应用相同的答案规范化逻辑。
...
默认通过 Hugging Face 的 OpenAI 兼容路由服务访问 `Qwen/Qwen3-VL-8B-Instruct`。基础 URL、API key 环境变量、模型、temperature、token 上限、结构化输出模式和执行后端都可配置。使用 Ray 后端时，执行节点需要能访问模型端点，并安装相同的 Vane AI 依赖。
