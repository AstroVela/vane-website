...
随后，Vane 的 Transformers provider 为原始片段文本生成嵌入。与其他嵌入示例相同，辅助函数会在接回嵌入之前验证行数和输出列。
...
终端会报告音频行数与字幕行数，再分别预览摘要行和已嵌入的片段行。完整向量用于内存中的 Relation，但 CSV 刻意只记录维度。

## 扩展与模型部署

转写与嵌入拥有独立的执行后端和批大小。对于 GPU 语音识别，actor 后端可以复用缓存的 Faster-Whisper 模型；嵌入阶段则可以使用另一组 actor 和资源配置。每个执行节点都必须能够导入可选依赖，并访问模型缓存或模型路径。
